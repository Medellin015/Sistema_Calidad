...
/* ===== 1. Configuración ===== */
const CARPETA_SGC_DEFAULT = 'https://activaparquesyeventos.sharepoint.com/sites/SistemadeGestindeCalidad/Documentos%20compartidos/Documentos%20Word/';
...
const MapaProcesos = ({ irA }) => (React.createElement("div", { className: "space-y-6" }, FRANJAS.map((f) => (React.createElement("section", { key: f.nombre },
    React.createElement("div", { className: "flex items-baseline gap-3 mb-2" },
        React.createElement("h3", { className: "f-display text-lg font-semibold" }, f.nombre),
        React.createElement("span", { className: "text-sm text-[#5b6b5f]" }, f.desc)),
    React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3" }, f.procesos.map((p) => {
        const n = DOCUMENTOS.filter((d) => d.proceso === p.sigla).length;
        return (React.createElement("button", { key: p.sigla, onClick: () => irA(`proceso/${p.sigla}`), className: "tarjeta text-left bg-white rounded-2xl border border-[#DCE5DC] p-4 flex items-center gap-3" },
            React.createElement("span", { className: "text-2xl", "aria-hidden": "true" }, p.icono),
            React.createElement("span", { className: "flex-1" },
                React.createElement("span", { className: "block font-semibold leading-snug" }, p.nombre),
                React.createElement("span", { className: "text-xs text-[#5b6b5f]" },
                    n,
                    " documento",
                    n !== 1 ? 's' : '')),
            React.createElement("span", { className: "f-mono text-xs font-bold text-[#1E6B47]" }, p.sigla)));
    })))))));
...
const VistaProceso = ({ sigla, irA }) => {
    ...
    return (React.createElement("div", { className: "max-w-3xl mx-auto" },
        ...
        guias.length > 0 && (React.createElement("div", { className: "mt-4 flex flex-wrap gap-2" }, guias.map((g) => (React.createElement("button", { key: g.id, onClick: () => irA(`guia/${g.id}`), className: "text-sm font-semibold bg-[#B5E048] text-[#14231B] rounded-full px-4 py-2 hover:bg-[#a3d13a]" }, g.pregunta))))),
        React.createElement("div", { className: "mt-6 space-y-2" }, docs.map((d) => React.createElement(FilaDocumento, { key: d.codigo + d.nombre, doc: d })))));
};
...
const Inicio = ({ irA }) => (React.createElement("div", { className: "max-w-4xl mx-auto" },
    React.createElement("div", { className: "pt-8 pb-6" },
        React.createElement("p", { className: "f-mono text-xs font-bold text-[#1E6B47] uppercase tracking-[.2em] mb-2" }, "Sistema de Gesti\u00F3n de Calidad"),
        React.createElement("h1", { className: "f-display text-4xl sm:text-6xl font-extrabold leading-[1.05]" },
            "\u00BFC\u00F3mo pido",
            React.createElement("br", null),
            React.createElement("span", { className: "text-[#1E6B47]" }, "lo que necesito"),
            " en ACTIVA?"),
        React.createElement("p", { className: "mt-3 text-[#3c4a40] max-w-xl leading-relaxed" },
            "Gu\u00EDas paso a paso con su flujograma, el mapa de procesos y los ",
            DOCUMENTOS.length,
    ...
    React.createElement("h2", { className: "f-display text-2xl font-extrabold mt-10 mb-4" }, "Mapa de procesos"),
